...
import { Home, BookOpen, Star, Trophy, LogOut, HeartPulse } from "lucide-react";
...
export default function StudentSidebar() {
  ...
  return (
    <div className="w-72 bg-white border-l border-gray-100 flex flex-col h-screen fixed top-0 right-0 z-50">
      <div className="p-8">
        ...
        {links.map((link) => {
          ...
          return (
            <Link
              key={link.href}
              href={link.href}
              className={`flex items-center space-x-3 space-x-reverse px-4 py-4 rounded-2xl font-bold transition-all ${
                isActive
      ...
      <div className="p-4 mt-auto">
        <div className="p-6 bg-indigo-600 rounded-3xl text-white relative overflow-hidden group mb-4">
          <div className="relative z-10">
            <p className="text-xs font-bold text-indigo-200 uppercase tracking-widest mb-1">الهدف الأسبوعي</p>
            <p className="text-xl font-black mb-4">3/5 جلسات مكتملة</p>
            <div className="w-full bg-indigo-500 h-2 rounded-full overflow-hidden">
              <div className="bg-white h-full w-[60%] rounded-full" />
            </div>
          </div>
          <Star className="absolute -bottom-4 -left-4 w-24 h-24 text-indigo-500 opacity-20 group-hover:scale-110 transition-transform" />
        </div>

        <button
          onClick={() => signOut()}
          className="w-full flex items-center space-x-3 space-x-reverse px-4 py-4 rounded-2xl font-bold text-gray-500 hover:bg-red-50 hover:text-red-600 transition-all"
        >
